...
detect_gpu() {
    ...
    if echo "$gpu_info" | grep -qi 'nvidia\|nvidia corporation'; then
        gpu_vendor="nvidia"; gpu_is_nvidia=true
        gpu_model=$(echo "$gpu_info" | grep -i nvidia | head -1 | sed 's/.*: //' | sed 's/ \[.*//' | cut -c1-80)
        if modinfo nvidia &>/dev/null; then gpu_driver="nvidia"
        elif modinfo nouveau &>/dev/null; then gpu_driver="nouveau"
        else gpu_driver="none"; fi

    elif echo "$gpu_info" | grep -qi 'amd\|advanced micro devices\|radeon\|amd/ati'; then
        gpu_vendor="amd"; gpu_is_amd=true
        gpu_model=$(echo "$gpu_info" | grep -i 'amd\|radeon' | head -1 | sed 's/.*: //' | sed 's/ \[.*//' | cut -c1-80)
        if modinfo amdgpu &>/dev/null; then gpu_driver="amdgpu"
        elif modinfo radeon &>/dev/null; then gpu_driver="radeon"
        else gpu_driver="none"; fi

    elif echo "$gpu_info" | grep -qi 'intel'; then
        gpu_vendor="intel"; gpu_is_intel=true
        gpu_model=$(echo "$gpu_info" | grep -i 'intel.*graphics\|intel.*hd\|intel.*iris' | head -1 | sed 's/.*: //' | sed 's/ \[.*//' | cut -c1-80)
        gpu_driver="modesetting"
        echo "  [Intel] Resmi destek yok - calisabilir ancak optimize degil"

    elif echo "$gpu_info" | grep -qi 'vmware\|virtualbox\|qemu\|vm'; then
        gpu_vendor="virtual"; gpu_model="Virtual Machine GPU"; gpu_driver="modesetting"
    fi
    ...
    cat > "$KARYA_HARDWARE_DIR/gpu.json" << JSONEOF
{
    ...
    "supported": $( [[ "$gpu_vendor" != "intel" ]] && echo "true" || echo "false" ),
    "note": $( [[ "$gpu_vendor" == "intel" ]] && echo "\"Intel GPU'lar resmi olarak desteklenmez. NVIDIA veya AMD onerilir.\"" || echo "\"\"" )
}
JSONEOF
...
detect_audio() {
    local audio_server="" has_pulse=false has_pipewire=false

    if pgrep -x pipewire &>/dev/null || command -v pipewire &>/dev/null; then
        has_pipewire=true; audio_server="pipewire"
...
detect_network() {
    ...
    if lsusb 2>/dev/null | grep -qi 'bluetooth\|bt'; then bt=true
    elif [[ -d /sys/class/bluetooth ]]; then bt=true; fi
...
generate_profile() {
    ...
    if [[ $ram -lt 4096 ]]; then
        ...
        profile="balanced"; compositor="opengl"; animations=true; blur=false
    else
        profile="performance"; compositor="opengl"; animations=true; blur=true
    fi

    case "$gpu_vendor" in
        nvidia) compositor="opengl"; if $is_laptop; then blur=false; fi ;;
        amd) compositor="opengl"; blur=true ;;
        intel) compositor="xrender"; blur=false ;;
        virtual) compositor="xrender"; animations=false; blur=false ;;
    esac
