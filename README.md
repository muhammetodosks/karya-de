...
**Karya Sun**, Karya DE'yi bir masaüstü ortamından **bulut bağlantılı bir ekosisteme** dönüştüren devasa altyapı güncellemesidir.

### 🚀 Sunucu Bileşenleri

| Bileşen | Teknoloji | İşlev |
|---------|-----------|-------|
| **Karya Sync** | Rust + WebSocket | Ayarlar, tema, notlar → bulut senkronizasyonu |
| **Karya Vault** | AES-256-GCM + Zero-Knowledge | Şifrelenmiş kullanıcı veri deposu |
| **Karya Relay** | STUN/TURN + QUIC | NAT aşan uzak masaüstü erişimi |
| **Karya Store** | GraphQL + CDN | Eklenti/tema pazaryeri arka ucu |
| **Karya Watch** | Prometheus + Loki | Sistem durumu izleme ve log toplama |

### 🌐 Öne Çıkan Özellikler

- **Çoklu Cihaz Senkronizasyonu** — Masaüstü ayarları, tema tercihleri, notlar ve yer imleri tüm cihazlar arasında anlık eşitlenir
- **Uzaktan Erişim** — Karya Relay ile güvenli, düşük gecikmeli uzak masaüstü (WebRTC + QUIC)
- **Kendi Sunucun** — Karya Sun'ı kendi altyapınızda çalıştırın; bulut bağımlılığı yok
- **E2E Şifreleme** — Zero-knowledge mimari: hiçbir veri sunucuda düz metin bulunmaz
- **Kurumsal SSO** — LDAP / Active Directory / OAuth2 entegrasyonu
- **Paket Depoları** — Karya Sun üzerinden özel APT/Pacman deposu yönetimi

### 📦 Ekosistem

```
karya-sun/
├── karya-sync/          # Senkronizasyon motoru (Rust)
├── karya-vault/         # Şifreli veri deposu (Go)
├── karya-relay/         # Uzaktan erişim relay (C++)
├── karya-store/         # Pazaryeri backend (Node.js)
├── karya-watch/         # Monitoring stack (Go)
├── deployment/
│   ├── docker/          # Docker Compose kurulumu
│   ├── kubernetes/      # Helm chart'ları
...
```bash
# Docker (önerilen)
curl -sL https://github.com/muhammetodosks/karya-de/raw/master/karya-sun/deploy.sh | bash

# Kubernetes
helm repo add karya-sun https://charts.karya-de.org
helm install karya-sun karya-sun/karya-sun

# Ansible (kendi sunucun)
ansible-playbook -i inventory.ini karya-sun/deployment/ansible/site.yml
```
...
|---------|--------------------|--------------------|
| **Senkronizasyon** | Yerel ayarlar | Çoklu cihaz + bulut |
| **Depolama** | Yerel disk | E2E şifreli bulut |
| **Erişim** | Yerel oturum | Uzaktan + Web |
| **Pazaryeri** | Yerel JSON | GraphQL + CDN |
| **İzleme** | Yok | Prometheus + Loki |
| **Güvenlik** | AppArmor + SELinux | Zero-Knowledge + SSO |
